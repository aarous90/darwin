...
public class WaterController : IController
{
	...
	private void Swim (float x, float y, bool seq)
	{
		currentCharacter.SetDirection (x, y);
	...
	public override void FixedUpdate ()
	{
		...
		if (L_Stick_Time != 0 && R_Stick_Time != 0) {
			if (Math.Abs (L_Stick_Time - R_Stick_Time) <= 0.1) {
				...
				Swim (Res_Stroke.x, Res_Stroke.y, true);
			...
			if (L_Stroke != Vector2.zero) {
				Swim (L_Stroke.x, L_Stroke.y, false);
				L_Stroke = Vector2.zero;
			}
			if (R_Stroke != Vector2.zero) {
				Swim (R_Stroke.x, R_Stroke.y, false);
				R_Stroke = Vector2.zero;
