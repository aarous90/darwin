...
public class _CharacterController : MonoBehaviour
{
	...
	void Update ()
	{
		move (new Vector2 (-1, -1) * Time.deltaTime);
	}
	...
	public void move (Vector2 moveDirection)
	{
		...
		for (int i = 0; i <= SideDivisions; i++) {
			...
			if (Physics.Raycast (new Vector2 (RayOriginX, RayOriginY), Vector2.up * direction, out hit, Mathf.Abs (deltaX))) {
		...
		for (int i = 0; i <= SideDivisions; i++) {
			...
			if (Physics.Raycast (new Vector2 (RayOriginX, RayOriginY), Vector2.right * direction, out hit, Mathf.Abs (deltaX))) {
				float distance = hit.distance;
				if (distance > skin) {
